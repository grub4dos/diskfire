...
{
	return (data->sblock.version & grub_cpu_to_be16_compile_time(
		XFS_SB_VERSION_NUMBITS)) ==
...
{
	if ((data->sblock.version & grub_cpu_to_be16_compile_time(
		XFS_SB_VERSION_NUMBITS)) ==
		grub_cpu_to_be16_compile_time(XFS_SB_VERSION_5) &&
		data->sblock.sb_features_incompat & grub_cpu_to_be32_compile_time(
			XFS_SB_FEAT_INCOMPAT_FTYPE))
		return 1;
	if (data->sblock.version & grub_cpu_to_be16_compile_time(
		XFS_SB_VERSION_MOREBITSBIT) &&
...
static int grub_xfs_sb_valid(struct grub_xfs_data* data)
{
	...
	else if ((data->sblock.version & grub_cpu_to_be16_compile_time(
		XFS_SB_VERSION_NUMBITS)) ==
		grub_cpu_to_be16_compile_time(XFS_SB_VERSION_4))
	{
		...
		if (data->sblock.version & grub_cpu_to_be16_compile_time(
			...
			(data->sblock.version & grub_cpu_to_be16_compile_time(
				XFS_SB_VERSION_MOREBITSBIT) &&
				data->sblock.features2 & grub_cpu_to_be16_compile_time(
	...
	struct grub_xfs_inode* inode)
{
	...
	grub_dprintf("xfs", "Reading inode (%" PRIuGRUB_UINT64_T ") - %"
		PRIuGRUB_UINT64_T ", %d\n",
		ino, block, offset);
	/* Read the inode.  */
	...
	if (node->inode.format == XFS_INODE_FORMAT_BTREE)
	{
		...
		do
		{
			...
			if (grub_disk_read(node->data->disk,
				GRUB_XFS_FSB_TO_BLOCK(node->data, get_fsb(keys,
					i - 1 + recoffset)) << (node->data->sblock.log2_bsize - GRUB_DISK_SECTOR_BITS),
	...
	grub_fshelp_iterate_dir_hook_t hook, void* hook_data)
{
	...
	case XFS_INODE_FORMAT_INO:
	{
		struct grub_xfs_dir_header* head = (struct grub_xfs_dir_header*)
			grub_xfs_inode_data(&diro->inode);
...
grub_xfs_get_inode_time(struct grub_xfs_inode* inode)
{
	...
	if (grub_xfs_inode_has_bigtime(inode))
		return (grub_be_to_cpu64(inode->mtime) / NSEC_PER_SEC) - XFS_BIGTIME_EPOCH_OFFSET;
...
#define XFS_UUID_STR "12345678-1234-1234-1234-12345678abcd"
...
grub_xfs_uuid(grub_disk_t disk, char** uuid)
{
	...
	if (data)
	{
		*uuid = grub_malloc(sizeof(XFS_UUID_STR));
		if (*uuid)
			grub_snprintf(*uuid, sizeof(XFS_UUID_STR), "%04x%04x-%04x-%04x-%04x-%04x%04x%04x",
