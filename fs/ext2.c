...
#define EXT2_BLOCK_SIZE(data)		(1ULL << LOG2_BLOCK_SIZE (data))
	...
	struct grub_ext2_block_group* blkgrp)
{
	...
	offset = (full_offset & ((1ULL << LOG2_BLOCK_SIZE(data)) - 1));
...
grub_ext2_read_block(grub_fshelp_node_t node, grub_disk_addr_t fileblock)
{
	...
	if (inode->flags & grub_cpu_to_le32_compile_time(EXT4_EXTENTS_FLAG))
	{
		...
		if (grub_ext4_find_leaf(data,
			(struct grub_ext4_extent_header*)inode->blocks.dir_blocks,
			(grub_uint32_t)fileblock, &leaf) != GRUB_ERR_NONE)
		{
	...
	int ino, struct grub_ext2_inode* inode)
{
	...
	if (grub_disk_read(data->disk,
		...
		((grub_off_t)blkoff) * EXT2_INODE_SIZE(data),
...
grub_ext2_mount(grub_disk_t disk)
{
	...
	if (data->sblock.revision_level != grub_cpu_to_le32_compile_time(
...
static grub_err_t
grub_ext2_dir(grub_disk_t disk, const char* path, grub_fs_dir_hook_t hook, void* hook_data)
{
...
#define EXT4_UUID_STR "12345678-1234-1234-1234-12345678abcd"
...
grub_ext2_uuid(grub_disk_t disk, char** uuid)
{
	...
	if (data)
	{
		*uuid = grub_malloc(sizeof(EXT4_UUID_STR));
		if (*uuid)
			grub_snprintf(*uuid, sizeof(EXT4_UUID_STR), "%04x%04x-%04x-%04x-%04x-%04x%04x%04x",
