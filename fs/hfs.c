	...
	grub_uint32_t idx, int type, char* datar, grub_size_t datalen)
{
	...
	do
	{
		...
		if (grub_hfs_iterate_records(data, type, idx, 0, grub_hfs_find_node_node_found,
			&ctx))
			return 0;
	...
	void* hook_arg)
{
	...
	do
	{
		...
		if (grub_hfs_iterate_records(data, 0, root_idx, 0,
			grub_hfs_iterate_dir_node_found, &ctx))
			return grub_errno;
	...
	grub_hfs_iterate_records(data, 0, ctx.next, 1, grub_hfs_iterate_dir_it_dir,
		&ctx);
	return grub_errno;
}
	...
	enum grub_fshelp_filetype* foundtype)
{
	...
	*foundtype = (fdrec.frec.type == GRUB_HFS_FILETYPE_DIR) ? GRUB_FSHELP_DIR :
		GRUB_FSHELP_REG;
	return GRUB_ERR_NONE;
}
	...
	void* hook_data)
{
	...
	grub_hfs_iterate_dir(data, data->cat_root, found->inode, grub_hfs_dir_hook,
		&ctx);
...
grub_hfs_open(struct grub_file* file, const char* name)
{
	...
	grub_memcpy(data->extents, found->fdrec.frec.extents,
...
#define HFS_UUID_STR "1234567812345678"
...
grub_hfs_uuid(grub_disk_t disk, char** uuid)
{
	...
	{
		*uuid = grub_malloc(sizeof(HFS_UUID_STR));
		if (*uuid)
			grub_snprintf(*uuid, sizeof(HFS_UUID_STR), "%016llx",
