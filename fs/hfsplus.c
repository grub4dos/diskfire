...
#undef c
	...
	key.attrkey.namelen = sizeof(compress_attr_name) / sizeof(
		compress_attr_name[0]);
	...
	if (cmp_head->type == grub_cpu_to_le32_compile_time(
		HFSPLUS_COMPRESSION_RESOURCE))
	{
		...
		node->cbuf_block = (grub_uint32_t)-1;
	...
	void* hook_arg)
{
	...
	grub_uint64_t saved_node = (grub_uint64_t)-1;
...
grub_hfsplus_label(grub_disk_t disk, char** label)
{
	...
	*grub_utf16_to_utf8((grub_uint8_t*)(*label), label_name, label_len) = '\0';
...
#define HFSPLUS_UUID_STR "1234567812345678"
...
grub_hfsplus_uuid(grub_disk_t disk, char** uuid)
{
	...
	if (data)
	{
		*uuid = grub_malloc(sizeof(HFSPLUS_UUID_STR));
		if (*uuid)
			grub_snprintf(*uuid, sizeof(HFSPLUS_UUID_STR), "%016llx",
