...
grub_f2fs_get_block(grub_fshelp_node_t node, grub_disk_addr_t block_ofs)
{
	...
	grub_uint32_t block_addr = (grub_uint32_t)-1;
	int level, i;
...
#define F2FS_UUID_STR "12345678-1234-1234-1234-12345678abcd"
...
grub_f2fs_uuid(grub_disk_t disk, char** uuid)
{
	...
	if (data)
	{
		*uuid = grub_malloc(sizeof(F2FS_UUID_STR));
		if (*uuid)
			grub_snprintf
			(*uuid, sizeof(F2FS_UUID_STR), "%02x%02x%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x%02x%02x%02x%02x",
