	...
	reiter:
		...
		if (head.level)
		{
			...
			*outaddr = 0;
			...
			if (desc)
				return save_ref(desc, addr - sizeof(head), (unsigned int)-1,
					grub_le_to_cpu32(head.nitems), 0);
			return GRUB_ERR_NONE;
		}
		{
	...
	grub_uint64_t csize, void* buf, grub_uint64_t parities_pos)
{
	...
	{
		/* Remove support for RAID rebuild */
	...
	grub_fs_dir_hook_t hook, void* hook_data)
{
	...
	do
	{
		...
		if (direl == NULL ||
			...
			/* FIXME: grub_sub(est_size, sizeof(direl->name), &est_size) || */
			grub_sub(est_size, 0, &est_size) ||
			est_size > allocated)
		{
				...
				+ grub_le_to_cpu16(cdirel->m)))
		{
			...
			if (cdirel == NULL ||
				...
				/* FIXME: grub_sub(est_size, sizeof(cdirel->name), &est_size) || */
				grub_sub(est_size, 0, &est_size) ||
				est_size > allocated)
			{
				grub_errno = GRUB_ERR_OUT_OF_RANGE;
...
#define BTRFS_UUID_STR "12345678-1234-1234-1234-12345678abcd"
...
grub_btrfs_uuid(grub_disk_t disk, char** uuid)
{
	...
	*uuid = grub_malloc(sizeof(BTRFS_UUID_STR));
	if (*uuid)
		grub_snprintf(*uuid, sizeof(BTRFS_UUID_STR), "%04x%04x-%04x-%04x-%04x-%04x%04x%04x",
