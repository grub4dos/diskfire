	...
	if (found != -1)
	{
			...
			sizeof(*tree), (char*)tree))
		{
			if (grub_memcmp(&tree->treehead, treehead, sizeof(struct grub_jfs_treehead))
				||
				grub_memcmp(&tree->extents, extents,
					254 * sizeof(struct grub_jfs_tree_extent)))
...
grub_jfs_getent(struct grub_jfs_diropen* diro)
{
	...
	le_to_cpu16_copy(filename + strpos, leaf->namepart,
		len < diro->data->namecomponentlen ? len
	...
	while (1)
	{
		...
		for (;;)
		{
			...
			if ((data->caseins ? grub_strncasecmp(name, diro->name, next - name) == 0
				: grub_strncmp(name, diro->name, next - name) == 0)
				&& !diro->name[next - name])
			{
...
#define JFS_UUID_STR "12345678-1234-1234-1234-12345678abcd"
...
grub_jfs_uuid(grub_disk_t disk, char** uuid)
{
	...
	if (data)
	{
		*uuid = grub_malloc(sizeof(JFS_UUID_STR));
		if (*uuid)
			grub_snprintf(*uuid, sizeof(JFS_UUID_STR),
