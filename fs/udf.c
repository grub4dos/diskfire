...
get_fshelp_size(struct grub_udf_data* data)
{
	...
	return sizeof(*x) + (1ULL << (GRUB_DISK_SECTOR_BITS + data->lbshift)) - sizeof(x->block);
}
	...
	struct grub_fshelp_node* node)
{
	...
	if (grub_disk_read(data->disk, block << data->lbshift, 0,
		1ULL << (GRUB_DISK_SECTOR_BITS + data->lbshift), &node->block))
		return grub_errno;
		...
		== GRUB_UDF_ICBTAG_FLAG_AD_SHORT)
	{
		...
		while (len >= (grub_ssize_t)sizeof(struct grub_udf_short_ad))
		{
			...
			if (adtype == 3)
			{
				...
				grub_disk_addr_t sec = grub_udf_get_block(node->data, (grub_uint16_t)node->part_ref, ad->position);
				if (!buf)
				{
			...
			if (filebytes < adlen)
			{
				...
				return ((U32(ad_pos) & GRUB_UDF_EXT_MASK) ? 0 :
					(grub_udf_get_block(node->data, (grub_uint16_t)node->part_ref, ad_pos)
		...
		while (len >= (grub_ssize_t)sizeof(struct grub_udf_long_ad))
		{
			...
			if (filebytes < adlen)
			{
				...
				return ((U32(ad_block_num) & GRUB_UDF_EXT_MASK) ? 0 :
					(grub_udf_get_block(node->data, (grub_uint16_t)ad_part_ref,
						ad_block_num)
						+ (filebytes >> (GRUB_DISK_SECTOR_BITS
							+ node->data->lbshift))));
			}
...
grub_udf_mount(grub_disk_t disk)
{
	...
	for (i = 0; i < data->npm; i++)
	{
		...
		for (j = 0; j < data->npd; j++)
			...
			{
				data->pms[i]->type1.part_num = (grub_uint16_t)j;
				break;
			}
...
gen_uuid_from_volset(char* volset_ident)
{
	...
	{
		for (i = 0; i < 8; ++i)
			uuid[i] = (char)grub_tolower(buf[i]);
...
grub_udf_uuid(grub_disk_t disk, char** uuid)
{
	...
	if (data)
	{
		volset_ident = read_dstring(data->pvd.volset_ident,
			sizeof(data->pvd.volset_ident));
		if (volset_ident)
		{
