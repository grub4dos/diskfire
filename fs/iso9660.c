	...
	void* hook_arg)
{
	...
	for (entry = (struct grub_iso9660_susp_entry*)sua;
		(char*)entry < (char*)sua + sua_size - 1 && entry->len > 0;
...
add_part(struct iterate_dir_ctx* ctx,
	...
	int len2)
{
	int size = ctx->symlink ? (int)grub_strlen(ctx->symlink) : 0;
	grub_size_t sz;
	char* new;
	...
	grub_fshelp_iterate_dir_hook_t hook, void* hook_data)
{
	...
	for (; offset < len; offset += dirent.len)
	{
...

		{
			...
			int nameoffset = (int)(offset + sizeof(dirent));
			struct grub_fshelp_node* node;
			int sua_off = (sizeof(dirent) + dirent.namelen + 1
				- (dirent.namelen % 2));
			int sua_size = dirent.len - sua_off;

			sua_off += (int)(offset + dir->data->susp_skip);
...
#define ISO9660_UUID_STR "1926-08-17-00-00-00-00"
static grub_err_t
grub_iso9660_uuid(grub_disk_t disk, char** uuid)
{
	...
	if (data)
	{
		...
		{
			*uuid = grub_malloc(sizeof(ISO9660_UUID_STR));
			if (*uuid)
				grub_snprintf(*uuid, sizeof(ISO9660_UUID_STR), "%c%c%c%c-%c%c-%c%c-%c%c-%c%c-%c%c-%c%c",
