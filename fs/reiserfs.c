	...
	struct grub_fshelp_node* item, int exact)
{
	...
	grub_uint16_t i;
	grub_uint16_t previous_level = (grub_uint16_t)~0;
	...
	grub_disk_read_hook_t read_hook, void* read_hook_data)
{
	...
	case GRUB_REISERFS_INDIRECT:
			...
			indirect_block++)
		{
			...
			grub_disk_read(found.data->disk,
				(grub_le_to_cpu32(indirect_block_ptr[indirect_block]) * block_size) /
...
#define FS_UUID_STR "12345678-1234-1234-1234-12345678abcd"
...
grub_reiserfs_uuid(grub_disk_t disk, char** uuid)
{
	...
	*uuid = NULL;
	...
	if (data)
	{
		...
		{
			*uuid = grub_malloc(sizeof(FS_UUID_STR));
			if (*uuid)
				grub_snprintf(*uuid, sizeof(FS_UUID_STR), "%04x%04x-%04x-%04x-%04x-%04x%04x%04x",
