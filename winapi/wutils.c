...
int push_perror(lua_State* L, const char* prefix) {
	char buff[512];
	sprintf_s(buff, 512, "%s: %s", prefix, last_error(0));
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	lpPrevWndProc = (WNDPROC)GetWindowLongPtrW(hwnd, GWLP_USERDATA);
	if (lpPrevWndProc)
		return CallWindowProcW(lpPrevWndProc, hwnd, uMsg, wParam, lParam);
...
void make_message_window() {
	if (hMessageWin == NULL) {
		...
		s_use_mutex = FALSE;
		hMessageWin = CreateWindowW(
			L"STATIC", L"winapi_Spawner_Dispatcher",
			0, 0, 0, 0, 0, 0, 0, GetModuleHandleW(NULL), 0
		);
		subclassedProc = SetWindowLongPtrW(hMessageWin, GWLP_WNDPROC, (LONG_PTR)WndProc);
		SetWindowLongPtrW(hMessageWin, GWLP_USERDATA, subclassedProc);
	}
}
