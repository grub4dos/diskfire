...
static int l_Window_get_text(lua_State* L) {
	Window* this = Window_arg(L, 1);
	GetWindowTextW(this->hwnd, wbuff, WBUFF);
...
static int l_Window_send_message(lua_State* L) {
	...
	lua_pushinteger(L, SendMessageW(this->hwnd, msg, (WPARAM)wparam, (LPARAM)lparam));
	return 1;
}
...
static int l_Window_post_message(lua_State* L) {
	...
	double wparam = luaL_checknumber(L, 3);
	double lparam = luaL_checknumber(L, 4);
	return push_bool(L, PostMessageW(this->hwnd, msg, (WPARAM)wparam, (LPARAM)lparam));
}
...
static int l_Window_get_module_filename(lua_State* L) {
	Window* this = Window_arg(L, 1);
	int sz = GetWindowModuleFileNameW(this->hwnd, wbuff, WBUFF);
	wbuff[sz / sizeof(wchar_t)] = 0;
	return push_wstring(L, wbuff);
}
...
static int l_Window_get_class_name(lua_State* L) {
	Window* this = Window_arg(L, 1);
	int n = GetClassNameW(this->hwnd, wbuff, WBUFF);
	if (n > 0) {
		push_wstring(L, wbuff);
		return 1;
	}
...
static int l_Window___tostring(lua_State* L) {
	Window* this = Window_arg(L, 1);
	int ret;
	int sz = GetWindowTextW(this->hwnd, wbuff, WBUFF);
	if (sz > MAX_SHOW) {
		wbuff[MAX_SHOW] = '\0';
	}
...
static int l_copy_file(lua_State* L) {
	...
	return push_bool(L, CopyFileA(src, dest, fail_if_exists));
}
...
static int l_move_file(lua_State* L) {
	const char* src = luaL_checklstring(L, 1, NULL);
	const char* dest = luaL_checklstring(L, 2, NULL);
	return push_bool(L, MoveFileA(src, dest));
}
...
static int l_open_serial(lua_State* L) {
	...
	*q = '\0';
	...
	hSerial = CreateFileW(wstring(port), GENERIC_READ | GENERIC_WRITE, 0, 0,
		OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
	if (hSerial == INVALID_HANDLE_VALUE) {
		return push_perror(L, "createfile");
	}
	GetCommState(hSerial, &dcb);
	if (!BuildCommDCBW(wstring(defn), &dcb)) {
		CloseHandle(hSerial);
...
static int l_event(lua_State* L) {
	...
	hEvent = CreateEventW(NULL, 0, 0, wstring(name));
	if (hEvent == NULL) {
		return push_error(L);
	}
	else {
...
static int l_mutex(lua_State* L) {
	const char* name = luaL_optlstring(L, 1, "", NULL);
	return push_new_Mutex(L, CreateMutexW(NULL, FALSE, *name == 0 ? NULL : wstring(name)));
}
...
static int l_Process_get_process_name(lua_State* L) {
	...
	if (EnumProcessModules(this->hProcess, &hMod, sizeof(hMod), &cbNeeded)) {
		if (full) {
			GetModuleFileNameExW(this->hProcess, hMod, modname, MAX_PATH);
		}
		else {
			GetModuleBaseNameW(this->hProcess, hMod, modname, MAX_PATH);
		}
...
static int l_File_write(lua_State* L) {
	...
	const char* s = luaL_checklstring(L, 2, NULL);    DWORD bytesWrote;
...
static void pipe_server_thread(PipeServerParms* parms) {
	while (1) {
		...
		HANDLE hPipe = CreateNamedPipeW(
			wstring(parms->pipename),             // pipe named
...
static int l_short_path(lua_State* L) {
	...
	res = GetShortPathNameW(wpath, wbuff, WBUFF);
	if (res > 0) {
		return push_wstring(L, wbuff);
	}
	else {
...
static int l_get_logical_drives(lua_State* L) {
	...
	LPWSTR p = dbuff;
	DWORD size = GetLogicalDriveStringsW(MAX_WPATH, dbuff);
	lua_newtable(L);
...
static int l_get_drive_type(lua_State* L) {
	...
	UINT res = GetDriveTypeW(wstring(root));
	const char* type = "?";
	switch (res) {
...
static int l_get_disk_free_space(lua_State* L) {
	...
	if (!GetDiskFreeSpaceExW(wstring(root), &freebytes, &totalbytes, NULL)) {
		return push_error(L);
	}
