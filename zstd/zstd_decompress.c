      ...
      case set_compressed:
                   ...
                   corruption_detected);   /* srcSize >= MIN_CBLOCK_SIZE == 3; here we need up to 5 for case 3 */
        {
          ...
          if (HUF_isError((litEncType==set_repeat) ?
                          ( singleStream ?
                            HUF_decompress1X_usingDTable_bmi2(dctx->litBuffer, litSize, istart+lhSize,
                                litCSize, dctx->HUFptr, dctx->bmi2) :
                            HUF_decompress4X_usingDTable_bmi2(dctx->litBuffer, litSize, istart+lhSize,
                                litCSize, dctx->HUFptr, dctx->bmi2) ) :
                          ( singleStream ?
                            HUF_decompress1X1_DCtx_wksp_bmi2(dctx->entropy.hufTable, dctx->litBuffer,
                                litSize, istart+lhSize, litCSize,
                                dctx->workspace, sizeof(dctx->workspace), dctx->bmi2) :
                            HUF_decompress4X_hufOnly_wksp_bmi2(dctx->entropy.hufTable, dctx->litBuffer,
                                litSize, istart+lhSize, litCSize,
                                dctx->workspace, sizeof(dctx->workspace), dctx->bmi2))))
            return ERROR(corruption_detected);
                   ...
                   unsigned tableLog)
{
  ...
  /* Build Decoding table */
  {
    ...
    for (u=0; u<tableSize; u++)
    {
      ...
      tableDecode[u].nextState = (U16) ( (nextState << tableDecode[u].nbBits) -
                                         tableSize);
                             ...
                             const void *src, size_t srcSize)
{
  ...
  *nbSeqPtr = nbSeq;
  ...
  if (dctx->ddictIsCold)
  {
    size_t const dictSize = (const char *)dctx->prefixStart -
                            (const char *)dctx->virtualStart;
  ...
  *litPtr = iLitEnd;   /* update for next sequence */
  ...
  else
  {
    ZSTD_wildcopy(op, match,
                  (ptrdiff_t)sequence.matchLength-8);   /* works even if matchLength < 8 */
  }
  return sequenceLength;
}
...
ZSTD_decodeSequence(seqState_t *seqState, const ZSTD_longOffset_e longOffsets)
{
  ...
  seq.matchLength = mlBase
                    + ((mlBits>0) ? BIT_readBitsFast(&seqState->DStream,
                        mlBits/*>0*/) : 0);  /* <=  16 bits */
  if (MEM_32bits()
  ...
  seq.litLength = llBase
                  + ((llBits>0) ? BIT_readBitsFast(&seqState->DStream,
                      llBits/*>0*/) : 0);    /* <=  16 bits */
  if (MEM_32bits())
    BIT_reloadDStream(&seqState->DStream);
                        ...
                        ZSTD_longOffset_e const longOffsets)
{
  ...
  seq.matchLength = mlBase + ((mlBits>0) ? BIT_readBitsFast(&seqState->DStream,
                              mlBits) : 0);  /* <=  16 bits */
  if (MEM_32bits()
      && (mlBits+llBits >= STREAM_ACCUMULATOR_MIN_32-LONG_OFFSETS_MAX_EXTRA_BITS_32))
    BIT_reloadDStream(&seqState->DStream);
...

  {
    ...
    seq.match = matchBase + pos -
                seq.offset;  /* note : this operation can overflow when seq.offset is really too large, which can only happen when input is corrupted.
...
#define ADVANCED_SEQS 4
          ...
          && (seqNb<nbSeq) ; seqNb++)
    {
      ...
      size_t const oneSeqSize = ZSTD_execSequenceLong(op, oend,
                                sequences[(seqNb-ADVANCED_SEQS) & STOSEQ_MASK], &litPtr, litEnd, prefixStart,
                                dictStart, dictEnd);
      if (ZSTD_isError(oneSeqSize))
        return oneSeqSize;
    ...
    {
      size_t const oneSeqSize = ZSTD_execSequenceLong(op, oend,
                                sequences[seqNb&STOSEQ_MASK], &litPtr, litEnd, prefixStart, dictStart, dictEnd);
      if (ZSTD_isError(oneSeqSize))
        return oneSeqSize;
      op += oneSeqSize;
    }
...
#if DYNAMIC_BMI2
  if (dctx->bmi2)
    return ZSTD_decompressSequences_bmi2(dctx, dst, maxDstSize, seqStart, seqSize,
                                         nbSeq, isLongOffset);
#endif
    ...
    const void *src, size_t srcSize, const int frame)
{
   ...
   */
  ZSTD_longOffset_e const isLongOffset = (ZSTD_longOffset_e)(MEM_32bits()
                                         && (!frame || dctx->fParams.windowSize > (1ULL << STREAM_ACCUMULATOR_MIN)));
...
static void ZSTD_checkContinuity(ZSTD_DCtx *dctx, const void *dst)
{
  if (dst != dctx->previousDstEnd)     /* not contiguous */
  {
    ...
    dctx->virtualStart = (const char *)dst - ((const char *)(
                           dctx->previousDstEnd) - (const char *)(dctx->prefixStart));
...
static size_t ZSTD_generateNxBytes(void *dst, size_t dstCapacity, BYTE value,
                                   size_t length)
{
                                        ...
                                        const ZSTD_DDict *ddict)
{
  ...
  while (srcSize >= ZSTD_frameHeaderSize_prefix)
  {
    ...
    if (ZSTD_isLegacy(src, srcSize))
    {
      ...
      decodedSize = ZSTD_decompressLegacy(dst, dstCapacity, src, frameSize, dict,
                                          dictSize);
...
{
  dctx->dictEnd = dctx->previousDstEnd;
  dctx->virtualStart = (const char *)dict - ((const char *)(
                         dctx->previousDstEnd) - (const char *)(dctx->prefixStart));
                               ...
                               const void *const dict, size_t const dictSize)
{
  ...
  ZSTD_STATIC_ASSERT(offsetof(ZSTD_entropyDTables_t,
                              OFTable) == offsetof(ZSTD_entropyDTables_t,
                                  LLTable) + sizeof(entropy->LLTable));
  ZSTD_STATIC_ASSERT(offsetof(ZSTD_entropyDTables_t,
                              MLTable) == offsetof(ZSTD_entropyDTables_t,
                                  OFTable) + sizeof(entropy->OFTable));
  ZSTD_STATIC_ASSERT(sizeof(entropy->LLTable) + sizeof(entropy->OFTable) + sizeof(
                       entropy->MLTable) >= HUF_DECOMPRESS_WORKSPACE_SIZE);
  {
    ...
    size_t const workspaceSize = sizeof(entropy->LLTable) + sizeof(
                                   entropy->OFTable) + sizeof(entropy->MLTable);
...

  {
    ...
    size_t const matchlengthHeaderSize = FSE_readNCount(matchlengthNCount,
                                         &matchlengthMaxValue, &matchlengthLog, dictPtr, dictEnd-dictPtr);
...

  {
    ...
    size_t const litlengthHeaderSize = FSE_readNCount(litlengthNCount,
                                       &litlengthMaxValue, &litlengthLog, dictPtr, dictEnd-dictPtr);
...
size_t ZSTD_decompressBegin_usingDDict(ZSTD_DCtx *dctx, const ZSTD_DDict *ddict)
{
  ...
  if (ddict)
  {
    dctx->ddictIsCold = (dctx->dictEnd != (const char *)ddict->dictContent +
                         ddict->dictSize);
...
ZSTD_DDict *ZSTD_createDDict(const void *dict, size_t dictSize)
{
  ...
  return ZSTD_createDDict_advanced(dict, dictSize, ZSTD_dlm_byCopy, ZSTD_dct_auto,
                                   allocator);
}
...
size_t ZSTD_estimateDStreamSize_fromFrame(const void *src, size_t srcSize)
{
  U32 const windowSizeMax = 1U <<
                            ZSTD_WINDOWLOG_MAX;   /* note : should be user-selectable */
  ZSTD_frameHeader zfh;
                             ...
                             ZSTD_inBuffer *input)
{
      ...
      case zdss_loadHeader :
        ...
        if (zds->legacyVersion)
        {
          ...
          if (zds->staticSize)
            return ERROR(memory_allocation);
          {
            size_t const hint = ZSTD_decompressLegacyStream(zds->legacyContext,
                                zds->legacyVersion, output, input);
            if (hint==0)
              zds->streamStage = zdss_init;
            return hint;
          }
...
#endif
        {
          size_t const hSize = ZSTD_getFrameHeader_advanced(&zds->fParams,
                               zds->headerBuffer, zds->lhSize, zds->format);
            ...
            && (U64)(size_t)(oend-op) >= zds->fParams.frameContentSize)
        {
          ...
          if (cSize <= (size_t)(iend-istart))
          {
            ...
            size_t const decompressedSize = ZSTD_decompress_usingDDict(zds, op, oend-op,
                                            istart, cSize, zds->ddict);
    ...
    zds->noForwardProgress = 0;
  {
    ...
    nextSrcSizeHint += ZSTD_blockHeaderSize * (ZSTD_nextInputType(
                         zds) == ZSTDnit_block);   /* preload header of next block */
