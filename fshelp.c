...
static grub_err_t
push_node(struct grub_fshelp_find_file_ctx* ctx, grub_fshelp_node_t node,
...
static grub_err_t
directory_find_file(grub_fshelp_node_t node, const char* name,
	grub_fshelp_node_t* foundnode,
	...
	for (name = currpath; ; name = next)
	{
		...
		*next = '\0';
		if (lookup_file)
			...
			err = directory_find_file(ctx->currnode->node, name, &foundnode, &foundtype,
				iterate_dir);
		*next = c;
