	...
	while (size--)
	{
		...
		if (code_high)
		{
			if (code >= 0xDC00 && code <= 0xDFFF)
			{
				...
				*dest++ = (grub_uint8_t)((code >> 18) | 0xF0);
				*dest++ = (grub_uint8_t)(((code >> 12) & 0x3F) | 0x80);
				*dest++ = (grub_uint8_t)(((code >> 6) & 0x3F) | 0x80);
				*dest++ = (grub_uint8_t)((code & 0x3F) | 0x80);
			}
			else
			{
				/* Error...  */
		...
		{
			if (code <= 0x007F)
				*dest++ = (grub_uint8_t)code;
			else if (code <= 0x07FF)
			{
				*dest++ = (grub_uint8_t)((code >> 6) | 0xC0);
				*dest++ = (grub_uint8_t)((code & 0x3F) | 0x80);
			}
			...
			{
				*dest++ = (grub_uint8_t)((code >> 12) | 0xE0);
				*dest++ = (grub_uint8_t)(((code >> 6) & 0x3F) | 0x80);
				*dest++ = (grub_uint8_t)((code & 0x3F) | 0x80);
			}
			else
