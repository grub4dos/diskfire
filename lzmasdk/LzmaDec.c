...
static int MY_FAST_CALL LzmaDec_DecodeReal(CLzmaDec *p, SizeT limit,
    const Byte *bufLimit)
{
  ...
  do
  {
    ...
    IF_BIT_0(prob)
    {
      ...
      {
        unsigned matchByte = p->dic[(dicPos - rep0) + ((dicPos < rep0) ? dicBufSize :
                                                    0)];
        unsigned offs = 0x100;
        symbol = 1;
        do
    ...
    else
    {
      ...
      if (state >= kNumStates)
      {
        ...
        if (checkDicSize == 0)
        {
          ...
          return SZ_ERROR_DATA;
        state = (state < kNumStates + kNumLitStates) ? kNumLitStates : kNumLitStates +
                3;
        /* state = kLiteralNextStates[state]; */
      }
